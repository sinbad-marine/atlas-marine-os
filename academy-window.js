...
function coreAnswerIsTrusted(data,envelope){return Boolean(String(data?.answer||'').trim()&&data.coreGateVersion===window.SinbadCore?.CORE_GATE_VERSION&&data.coreGateVersion===envelope?.gateVersion&&data.permission==='DECISION_SUPPORT_ONLY'&&data.executionPerformed===false)}
