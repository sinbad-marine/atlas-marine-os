...
function setInstructorState(state,status,detail={}){const supported=window.SinbadCharacterEngine?.STATES?.includes?.(state),safeState=supported?state:(AVATARS[state]?state:'idle');academyPerformanceDirector?.cancel();if(academyCharacterEngine)academyCharacterEngine.setState(safeState,detail);else renderCharacterSnapshot({state:safeState,gesture:detail.gesture||'rest',gaze:detail.gaze||'audience',emotion:detail.emotion||'warm'});if(status)byId('academyInstructorStatus').textContent=status}
function setThinkingStage(stage,status){const result=window.SinbadPerformanceDirector?.thinkingCueForStage?.(stage);setInstructorState('thinking',status,{...(result?.accepted?result.cue:{}),thinkingStage:stage})}
...
function speak(text,{afterState='teaching',afterStatus='Lesson ready'}={}){lastNarration=String(text||'').trim();syncVoiceControls();if(!lastNarration||!voiceEnabled)return;if(!('speechSynthesis'in window)||!('SpeechSynthesisUtterance'in window)){setInstructorState('teaching','Voice is unavailable in this browser');return}const run=++narrationRun,chunks=narrationChunks(lastNarration),voice=selectVoice();window.speechSynthesis.cancel();byId('academyStopVoice').disabled=false;const opening=window.SinbadPerformanceDirector?.responseCueForText?.(lastNarration,'instructional');setInstructorState('speaking','Captain Sinbad is speaking',opening?.accepted?opening.cue:{});const play=index=>{if(run!==narrationRun)return;if(index>=chunks.length){byId('academyStopVoice').disabled=true;setInstructorState(afterState,afterStatus);return}const spokenText=chunks[index],utterance=new SpeechSynthesisUtterance(spokenText);utterance.lang=voice?.lang||'tr-TR';utterance.voice=voice;utterance.rate=.94;utterance.pitch=1;utterance.volume=1;utterance.onboundary=event=>{if(run!==narrationRun)return;const cue=window.SinbadPerformanceDirector?.speechCueForBoundary?.({text:spokenText,name:event.name||'word',charIndex:event.charIndex||0,wordIndex:Math.max(0,event.charIndex||0),mode:'instructional'});if(cue?.accepted)setInstructorState('speaking','Captain Sinbad is speaking',cue.cue)};utterance.onend=()=>play(index+1);utterance.onerror=event=>{if(event.error!=='canceled'){byId('academyStopVoice').disabled=true;setInstructorState('teaching','Voice playback could not continue')}};window.speechSynthesis.speak(utterance)};play(0)}
...
async function askLocalSinbad(question,history,coreEnvelope){const response=await fetch(`${SINBAD_BRIDGE_URL}/ai/chat`,{method:'POST',headers:{'Content-Type':'application/json',...argosBridgeHeaders('AI_INFERENCE','/ai/chat')},body:JSON.stringify({question,language:'tr-TR',history,coreEnvelope})});if(!response.ok)throw new Error(`Yerel Sinbad motoru ${response.status} döndürdü.`);const data=await response.json(),answer=String(data?.answer||'').trim();if(!answer||window.SinbadCoreDecision?.answerIsSafe?.(answer)!==true||coreEnvelope?.gateVersion!==window.SinbadCore?.CORE_GATE_VERSION)throw new Error('Yerel Sinbad güvenlik kapısından geçemedi.');return {answer,model:data.model||'local-model',mode:data.mode||'offline-local-ai'}}
async function askCloudSinbad(question,history,coreEnvelope){const workspaceId=localStorage.getItem('atlas_selected_workspace')||localStorage.getItem('atlas-v81-workspace')||'';if(!cloudClient||!cloudSession?.user||!workspaceId)throw new Error('Atlas Cloud oturumu veya çalışma alanı bağlı değil.');const {data,error}=await cloudClient.functions.invoke('sinbad-answer',{body:{workspaceId,question,language:'tr-TR',includeSourceVisuals:false,suppressSourceVisuals:true,coreEnvelope}});if(error)throw error;if(!coreAnswerIsTrusted(data,coreEnvelope))throw new Error('Sinbad güvenlik/kanıt kapısı bu cevabı durdurdu.');const answer=String(data.answer).trim();return {answer,model:'atlas-cloud',mode:'cloud-rag'}}
async function askSinbad(question){const history=messages.slice(0,-1).slice(-12).map(message=>({role:message.role==='sinbad'?'assistant':'user',content:message.text})),coreEnvelope=window.SinbadCore?.aiEnvelope?.(question,history);let result;try{setThinkingStage('retrieving','Yerel Sinbad hafızasına erişiyor');result=await askLocalSinbad(question,history,coreEnvelope)}catch(localError){console.warn('Academy local brain unavailable',localError);setThinkingStage('retrieving','Atlas Cloud kaynaklarına erişiyor');result=await askCloudSinbad(question,history,coreEnvelope)}setThinkingStage('composing','Sinbad cevabı oluşturuyor');const answer=result.answer,visuals=await window.SinbadVisuals?.select?.(question,answer,{max:3})||[];return {...result,visuals}}
async function submitQuestion(question){const clean=String(question||'').trim();if(!clean)return;stopNarration('Thinking');addMessage('user',clean);byId('academyQuestion').value='';byId('academySend').disabled=true;setThinkingStage('analyzing','Sinbad sorunuzu anlamlandırıyor');byId('academyCloudStatus').textContent='Yerel zekâ ve Atlas kütüphanesi birlikte çalışıyor…';try{const result=await askSinbad(clean);addMessage('sinbad',result.answer,result.visuals);byId('academyCloudStatus').textContent=`${result.model} · ${result.visuals.length?`${result.visuals.length} doğrulanmış görsel`:'görsel eşleşmesi yok'}`;speak(result.answer,{afterState:'listening',afterStatus:'Captain Sinbad is ready for your next question'})}catch(error){addMessage('sinbad',`Bu soruyu şu anda tamamlayamadım: ${error.message||error}`);byId('academyCloudStatus').textContent='Answer stopped safely';setInstructorState('error','Bağlı bir zekâ motoruna ihtiyacım var',{emotion:'concerned',gesture:'hold',gaze:'audience'})}finally{byId('academySend').disabled=false;byId('academyQuestion').focus()}}
