(function(root,factory){
  ...
  function academyBoardShapeCheckAnswerForText(text,check,language='tr-TR'){
    ...
    return Object.freeze({accepted:true,correct,...(!correct?{retry:true}:{}),text:correct?(turkish?'Doğru. Cevabın tahtadaki şekille uyuşuyor.':'Correct. Your answer matches the shape on the board.'):(turkish?`Bu cevap doğrulanmış şekil özelliğiyle uyuşmuyor. ${correction[check.expected]} Bir kez daha deneyebilirsin.`:`That answer does not match the verified shape property. ${correction[check.expected]} You can try once more.`)});
  }
