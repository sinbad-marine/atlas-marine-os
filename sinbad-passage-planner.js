(function(root,factory){
  ...
  function checklist(plan){
    return [`PASSAGE PLAN DRAFT — ${plan.name}`,'DRAFT — MASTER APPROVAL AND OFFICIAL-SOURCE VERIFICATION REQUIRED','',`Total distance: ${plan.totalDistanceNm.toFixed(1)} NM`,`Plan speed: ${plan.speedKn.toFixed(1)} kn`,`Estimated duration: ${plan.totalHours.toFixed(1)} hours`,`Estimated fuel including margin: ${Math.ceil(plan.fuelRequiredLitres)} L`,'','APPRAISAL','• Verify current official charts, NtM, MSI/NAVTEX, weather, tide and current data','• Verify draft, UKC, squat, air draft, restricted waters, port and pilotage requirements','','PLANNING',...plan.legs.map(item=>`• Leg ${item.number}: ${item.from.name} → ${item.to.name} | ${item.courseTrue.toFixed(0).padStart(3,'0')}°T | ${item.distanceNm.toFixed(2)} NM | ${item.hours.toFixed(2)} h${item.eta?` | ETA ${item.eta}`:''}`),'','EXECUTION / MONITORING','• Record the bridge briefing, abort point, wheel-over, XTE and no-go limits separately','• Monitor position, traffic, machinery, weather, ETA and fuel deviations throughout the voyage','• This automated draft must not be used as the sole means of navigation.'].join('\n');
  }
