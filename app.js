...
async function sendToSinbad(text){
  ...
  if(sinbadPendingAcademyBoardCheck){const hint=sinbadPerformanceDirector?.academyBoardShapeCheckHintForText?.(q,sinbadPendingAcademyBoardCheck,sinbadState.language||appLanguage);if(hint?.accepted){if(hint.hint===true)sinbadPendingAcademyBoardCheck=Object.freeze({...sinbadPendingAcademyBoardCheck,hintsUsed:(sinbadPendingAcademyBoardCheck.hintsUsed||0)+1});presentSinbadBoardReference(hint.text);return;}const repeatedQuestion=sinbadPerformanceDirector?.academyBoardShapeCheckRepeatForText?.(q,sinbadPendingAcademyBoardCheck,sinbadState.language||appLanguage);if(repeatedQuestion?.accepted){presentSinbadBoardReference(repeatedQuestion.text);return;}const reasoned=sinbadPerformanceDirector?.academyBoardShapeCheckReasonForText?.(q,sinbadPendingAcademyBoardCheck,sinbadState.language||appLanguage);if(reasoned?.accepted){presentSinbadBoardReference(reasoned.text);return;}const revealed=sinbadPerformanceDirector?.academyBoardShapeCheckRevealForText?.(q,sinbadPendingAcademyBoardCheck,sinbadState.language||appLanguage);if(revealed?.accepted){sinbadLastAcademyBoardCheck=Object.freeze({shape:sinbadPendingAcademyBoardCheck.shape,expected:sinbadPendingAcademyBoardCheck.expected});sinbadPendingAcademyBoardCheck=null;presentSinbadBoardAssessment(revealed);return;}const assessed=sinbadPerformanceDirector?.academyBoardShapeCheckAnswerForText?.(q,sinbadPendingAcademyBoardCheck,sinbadState.language||appLanguage);if(assessed?.accepted){if(assessed.retry===true)sinbadPendingAcademyBoardCheck=Object.freeze({...sinbadPendingAcademyBoardCheck,attempts:(sinbadPendingAcademyBoardCheck.attempts||0)+1});else{sinbadLastAcademyBoardCheck=assessed.cancelled?null:Object.freeze({shape:sinbadPendingAcademyBoardCheck.shape,expected:sinbadPendingAcademyBoardCheck.expected});sinbadPendingAcademyBoardCheck=null;}presentSinbadBoardAssessment(assessed);return;}}
