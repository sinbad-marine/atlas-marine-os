...
async function sendToSinbad(text){
  ...
  if(sinbadPendingAcademyBoardCheck){const assessed=sinbadPerformanceDirector?.academyBoardShapeCheckAnswerForText?.(q,sinbadPendingAcademyBoardCheck,sinbadState.language||appLanguage);if(assessed?.accepted){if(assessed.correct===true||assessed.cancelled===true)sinbadPendingAcademyBoardCheck=null;presentSinbadBoardAssessment(assessed);return;}}
