...
test('fails closed instead of silently truncating an oversized review',()=>{
  const source=require('node:fs').readFileSync(require('node:path').resolve(__dirname,'../tools/roundtable-review.js'),'utf8');
  assert.match(source,/Review diff exceeds/);
  assert.doesNotMatch(source,/DIFF TRUNCATED/);
});
