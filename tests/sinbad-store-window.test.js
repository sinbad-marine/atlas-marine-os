...
test('dashboard launches Store in one resizable independent window',()=>{assert.match(app,/openDashboardWorkspaceWindow/);assert.match(app,/new URL\('\.\/store\.html',location\.href\)/);assert.match(app,/sinbadMarineStore/);assert.match(app,/resizable=yes,scrollbars=yes/)});
