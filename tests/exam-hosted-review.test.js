...
test('hosted Owner review uses the same-origin authenticated surface instead of loopback',()=>{assert.match(launcher,/new URL\('\.\/exam-review\.html',runtimeUrl\)/);assert.doesNotMatch(html,/127\.0\.0\.1:4177/);assert.match(client,/auth\.getSession\(\)/);assert.match(client,/invokeFunction\('exam-answer-key-review'/);assert.match(client,/invokeFunction\('human-review'/)});
