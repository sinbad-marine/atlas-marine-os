...
test('full Bridge boots with isolated data and rejects every protected write without Owner proof',{skip:process.platform!=='win32',timeout:60000},async t=>{
 ...
 const preflight=await fetch(base+'/routes',{method:'OPTIONS',headers:{Origin:'https://sinbad-marine.github.io','Access-Control-Request-Method':'POST','Access-Control-Request-Headers':'authorization,content-type,x-sinbad-owner-authorization,x-sinbad-owner-nonce'},signal:AbortSignal.timeout(5000)});
 assert.equal(preflight.status,204);assert.equal(preflight.headers.get('access-control-allow-origin'),'https://sinbad-marine.github.io');assert.match(preflight.headers.get('access-control-allow-headers'),/X-Sinbad-Owner-Nonce/);
