...
test('hosted Academy opens the same-origin authenticated review surface',()=>{const opened=[];const integration=ownerReview.create({appUrl:'http://127.0.0.1:4177/'},{baseUrl:'https://sinbad-marine.github.io/atlas-marine-os/academy.html',openWindow:(...args)=>{opened.push(args);return {};}});assert.equal(integration.launch().safeFallback,true);assert.equal(opened[0][0],'https://sinbad-marine.github.io/atlas-marine-os/exam-review.html');assert.equal(opened[0][0].includes('127.0.0.1'),false);});
