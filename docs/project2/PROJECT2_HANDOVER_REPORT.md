...
- **Query expansion — measured on a blind set, NOT ACCEPTED (Phase 4.9, Owner directive REVISE of 2026-09-19).** The old TEST split is frozen as a USED HOLDOUT; a new blind set of 36 probes was judged once, against criteria committed beforehand (`ac74417`). The installed `qwen3:4b` is thinking-only and unusable for this (0 of 24); `qwen3:14b` was used, nothing was installed. The original question is never replaced (the first 4 of 6 passages stay the untouched lexical result) and an expansion that states a number or a regulation reference the question did not contain is discarded whole. Result: hits 22 → 24 of 36, strict 12 → 14 of 25, 3 gains, 1 regression — below the preregistered thresholds, so **it is not wired**. The more important number: the baseline finds the passage that answers for only **12 of 25 strict probes (48 %)** on unseen questions. Lexical retrieval is the weak link. See `docs/project2/PHASE_4_9_QUERY_EXPANSION.md`.
...
| 8 | Installing an embedding model on the Owner's machine for a semantic re-ranker or a semantic index (Phase 4.8 / 4.9) | Nothing was installed. The alternative that needs no installation (question restated by an installed local model) was measured on a blind set and NOT ACCEPTED, so this is now the open decision. A further candidate needs a new blind set: both held-out sets are used. |
| 9 | OWNER ACCEPTED for 4.8 and 4.9 | NOT RECORDED. |
