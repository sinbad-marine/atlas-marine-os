...
test('module exposes authenticated opening for isolated recovery without execution authority',()=>{assert.deepEqual(Object.keys(archive).sort(),['ALGORITHM','VERSION','createArchive','openArchive','verifyArchive']);for(const name of Object.keys(archive))assert.doesNotMatch(name,/upload|network|deploy|restore|delete|credential/iu);});
