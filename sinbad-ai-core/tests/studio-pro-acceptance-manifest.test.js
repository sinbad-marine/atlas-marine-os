...
test('freezes the finite honest Pro 0.3.1 acceptance scope',()=>{
  ...
  for(const capability of ['LOOPBACK_ONLY_LOCAL_MODEL_PROTOCOL','STRICT_MODEL_ARTIFACT_JSON_VALIDATION','ISOLATED_UNTRUSTED_PROPOSAL_PERSISTENCE','SCRIPTLESS_MODEL_PREVIEW_PACKAGING','DIGEST_PINNED_OLLAMA_ISOLATION_PLAN'])assert.ok(manifest.acceptedCapabilities.includes(capability),capability);
  ...
  assert.match(manifest.runtimeEvidence.containerImage,/^ollama\/ollama@sha256:[a-f0-9]{64}$/u);assert.equal(manifest.runtimeEvidence.containerImagePresentLocally,true);assert.equal(manifest.runtimeEvidence.containerStarted,false);
});
