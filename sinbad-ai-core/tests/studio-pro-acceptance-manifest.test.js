...
test('freezes the finite honest Pro 0.3.4 acceptance scope',()=>{
  ...
  assert.equal(manifest.runtimeEvidence.lowResourceCandidate.model,'qwen3:4b');assert.match(manifest.runtimeEvidence.lowResourceCandidate.digest,/^sha256:[a-f0-9]{64}$/u);assert.equal(manifest.runtimeEvidence.lowResourceCandidate.license,'Apache-2.0');assert.equal(manifest.runtimeEvidence.lowResourceCandidate.downloaded,true);assert.equal(manifest.runtimeEvidence.lowResourceCandidate.modelCalled,false);
});
test('keeps execution remote access merge Core write and publish explicitly prohibited',()=>{
  ...
  assert.equal(manifest.activationBlockers[0].code,'LOCAL_MODEL_CONTAINER_INFERENCE_DID_NOT_COMPLETE');assert.ok(manifest.activationBlockers[0].blocks.includes('UNATTENDED_MODEL_INFERENCE'));
});
