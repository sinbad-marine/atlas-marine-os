...
test('freezes the finite honest Pro 0.3.2 acceptance scope',()=>{
...
test('keeps execution remote access merge Core write and publish explicitly prohibited',()=>{
  ...
  assert.equal(manifest.activationBlockers[0].code,'LOCAL_MODEL_INFERENCE_NOT_YET_REPRODUCED_IN_SANDBOX');assert.ok(manifest.activationBlockers[0].blocks.includes('UNATTENDED_MODEL_INFERENCE'));
});
