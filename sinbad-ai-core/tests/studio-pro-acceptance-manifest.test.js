...
test('freezes the finite honest Pro 0.3.6 acceptance scope',()=>{
  ...
  assert.equal(manifest.runtimeCompatibility.dockerDesktopDevice,'/dev/dxg');assert.equal(manifest.runtimeCompatibility.dockerDesktopDriVisible,false);assert.match(manifest.runtimeCompatibility.ollamaKnownIssue,/github\.com\/ollama\/ollama\/issues\/14854/u);assert.equal(manifest.runtimeCompatibility.decision,'DO_NOT_FORCE_OLLAMA_VULKAN_ON_THIS_WSL2_STACK');
});
test('keeps execution remote access merge Core write and publish explicitly prohibited',()=>{
  ...
  assert.equal(manifest.activationBlockers[0].code,'OLLAMA_WSL2_INTEL_ARC_140T_COMPLETION_STALL');assert.ok(manifest.activationBlockers[0].blocks.includes('UNATTENDED_MODEL_INFERENCE'));
});
