...
test('advertises an exact frozen reconciliation audit row contract', () => {
...
test('audit row accessors fail closed without invocation', () => {
...
test('descriptor traps inherited and coercive audit fields fail closed', () => {
  let calls = 0;
  const malicious = { toString() { calls++; throw new Error('must not run'); }, valueOf() { calls++; throw new Error('must not run'); } };
  for (const value of [Object.create(row()), new Proxy(row(), { getOwnPropertyDescriptor() { throw new Error('host failure'); } }), { ...row(), id: malicious }, { ...row(), actor_hash: malicious }, { ...row(), decided_at_ms: '1001' }]) assert.equal(verifier.parse([value]), null);
