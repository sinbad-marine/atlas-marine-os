...
test('creates a deeply immutable grounded answer contract',()=>{
  const result=contracts.groundedAnswer({status:'GROUNDED',answer:'Fact.',claims:[{id:'c1',text:'Fact.',evidenceIds:['e1'],citationIds:['x'],supported:true}],citations:[{id:'x',claimId:'c1',evidenceId:'e1'}],confidence:{state:'HIGH',reasons:['SUPPORTED']}});
  assert.equal(result.version,'sinbad-grounded-answer/2B');
