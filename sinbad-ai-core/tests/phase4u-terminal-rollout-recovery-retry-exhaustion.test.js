...
test('last nonterminal attempt exhausts without an additional scheduling clock read or retained deadline', async () => {
  ...
  assert.deepEqual(value.inspect(deployment), { version: lifecycle.LIFECYCLE_VERSION, status: 'ROLLOUT_RECOVERY_DEPLOYMENT_LIFECYCLE_STATE', phase: 'RETRY_EXHAUSTED', attemptsUsed: 1, attemptsRemaining: 0, retryNotBefore: null });
...
test('advertises terminal retry exhaustion semantics', () => {
  assert.equal(lifecycle.LIFECYCLE_VERSION, 'sinbad-rollout-recovery-deployment-lifecycle-runtime/4U-v1');
});
