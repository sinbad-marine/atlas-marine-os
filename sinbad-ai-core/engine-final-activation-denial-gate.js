...
function assess(input) {
  try {
    ...
    const receiptHashes = KEYS.slice(2).map(key => value[key]);
    if (value.version !== VERSION || !ID.test(value.engineId) || receiptHashes.some(hash => typeof hash !== 'string' || !HASH.test(hash))) return denied('ENGINE_ACTIVATION_REQUEST_INVALID', ['BOUND_VERIFIER_RECEIPTS_REQUIRED']);
    if (new Set(receiptHashes).size !== receiptHashes.length) return denied('ENGINE_ACTIVATION_RECEIPT_ROLE_COLLISION', ['DISTINCT_REGISTRATION_REVOCATION_POLICY_VALIDATION_AND_ISOLATION_RECEIPTS_REQUIRED']);
