...
const distinctHashes = (value, fields) => fields.every(field => hash(value[field])) && new Set(fields.map(field => value[field])).size === fields.length;
function snapshot(kind, input) {
  ...
  if (kind === 'OperationalEnvelope') valid = id(value.envelopeId) && id(value.vesselId) && safeTime(value.validFrom) && safeTime(value.validUntil) && value.validUntil > value.validFrom && typeof value.mode === 'string' && MODES.has(value.mode) && distinctHashes(value, ['constraintsHash', 'approvalHash']);
  if (kind === 'CommandIntent') valid = id(value.intentId) && id(value.vesselId) && distinctHashes(value, ['actorHash', 'nonceHash', 'commandScopeHash', 'timeAuthorityHash', 'parametersHash', 'operationalEnvelopeHash']) && id(value.idempotencyKey) && safeTime(value.notBefore) && safeTime(value.issuedAt) && safeTime(value.expiresAt) && value.notBefore <= value.issuedAt && value.expiresAt > value.issuedAt && Number.isSafeInteger(value.revocationEpoch) && value.revocationEpoch >= 0 && typeof value.action === 'string' && ACTIONS.has(value.action) && typeof value.requiresDualApproval === 'boolean';
  if (kind === 'SafetyDecision') valid = distinctHashes(value, ['intentHash', 'kernelHash']) && typeof value.decision === 'string' && DECISIONS.has(value.decision) && id(value.reasonCode) && safeTime(value.decidedAt);
  if (kind === 'ControlExecutionReceipt') valid = distinctHashes(value, ['intentHash', 'decisionHash', 'equipmentHash']) && typeof value.status === 'string' && RECEIPTS.has(value.status) && safeTime(value.recordedAt);
