...
The permanent long-term mission, future vessel-control capability envelope, and non-negotiable safety invariants are defined in [SINBAD_CORE_CHARTER.md](./SINBAD_CORE_CHARTER.md).
...
Phase 5U snapshots terminal rollout recovery timeout, resolver, diagnostics, and durable journal methods from direct own data properties. Accessors, descriptor traps, inherited methods, numeric strings, bigint, coercive values, and later dependency mutation fail closed or remain isolated.
...
Phase 5X snapshots authorization-audit append configuration and each recorded event from exact own primitive data. Accessors, descriptor traps, inherited fields, coercive values, unsafe timestamps, and post-construction append mutation fail closed or remain isolated.
...
Phase 6G hardens the reconciliation audit Supabase adapter itself. Service-role/RPC configuration and appended audit events are captured from exact own data once; accessor, inherited, coercive, or mutating inputs fail closed before transport, and RPC receives a frozen primitive payload.

Phase 6H hardens the reconciliation audit verifier transport boundary. Service-role/RPC configuration, capability/list response envelopes, and every array index are admitted from exact own data; accessor, inherited, sparse, coercive, or mutating responses fail closed, and RPC arguments are frozen.
