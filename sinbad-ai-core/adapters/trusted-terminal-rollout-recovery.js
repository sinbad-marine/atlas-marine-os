...
function journalSnapshot(value){if(!value||typeof value!=='object')return null;const output=Object.create(null);for(const name of ['version','durable','inspect','settle']){let descriptor;try{descriptor=Object.getOwnPropertyDescriptor(value,name);}catch{return null;}if(!descriptor||!Object.hasOwn(descriptor,'value'))return null;output[name]=descriptor.value;}if(output.version!==EXPECTED_JOURNAL_VERSION||output.durable!==true||typeof output.inspect!=='function'||typeof output.settle!=='function')return null;output.inspect=output.inspect.bind(value);output.settle=output.settle.bind(value);return Object.freeze(output);}
function policy(options){if(!options||typeof options!=='object')return null;const output=Object.create(null);for(const name of ['activationJournal','resolve','diagnose','recoveryTimeoutMs']){let descriptor;try{descriptor=Object.getOwnPropertyDescriptor(options,name);}catch{return null;}if(descriptor!==undefined&&!Object.hasOwn(descriptor,'value'))return null;output[name]=descriptor===undefined?undefined:descriptor.value;}output.activationJournal=journalSnapshot(output.activationJournal);if(!output.activationJournal||typeof output.resolve!=='function'||(output.diagnose!==undefined&&typeof output.diagnose!=='function')||!Number.isSafeInteger(output.recoveryTimeoutMs)||output.recoveryTimeoutMs<1000||output.recoveryTimeoutMs>300000)return null;return Object.freeze(output);}

function create(options={}){
  const admitted=policy(options);if(!admitted){let raw;try{raw=Object.create(null);for(const name of ['activationJournal','resolve','diagnose','recoveryTimeoutMs']){const descriptor=options&&typeof options==='object'?Object.getOwnPropertyDescriptor(options,name):null;if(descriptor&&!Object.hasOwn(descriptor,'value'))throw new Error('accessor');raw[name]=descriptor?descriptor.value:undefined;}}catch{throw new TypeError('A trusted bounded recovery policy is required');}if(!journalSnapshot(raw.activationJournal))throw new TypeError('A trusted durable activationJournal is required');if(typeof raw.resolve!=='function')throw new TypeError('A trusted resolve function is required');if(raw.diagnose!==undefined&&typeof raw.diagnose!=='function')throw new TypeError('diagnose must be a function');if(!Number.isSafeInteger(raw.recoveryTimeoutMs)||raw.recoveryTimeoutMs<1000||raw.recoveryTimeoutMs>300000)throw new TypeError('A bounded recovery timeout is required');throw new TypeError('A trusted bounded recovery policy is required');}
  const journal=admitted.activationJournal,timeoutMs=admitted.recoveryTimeoutMs,resolve=admitted.resolve,diagnose=admitted.diagnose||(()=>{});
