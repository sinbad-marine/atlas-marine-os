...
const ADAPTER_VERSION='sinbad-trusted-terminal-delivery-adapter/3A-v1';
...
function reconciled(input={}){return Object.freeze({version:ADAPTER_VERSION,status:'TRUSTED_TERMINAL_DELIVERY_RECONCILED',reasonCode:null,terminalState:input.terminalState||null,outcome:['DELIVERED','FAILED'].includes(input.outcome)?input.outcome:null,sourceCount:Number.isInteger(input.sourceCount)?input.sourceCount:0,completionHash:/^[a-f0-9]{64}$/u.test(input.completionHash||'')?input.completionHash:null,transitionHash:/^[a-f0-9]{64}$/u.test(input.transitionHash||'')?input.transitionHash:null});}
...
function create(options={}){
  ...
  const inFlightKeys=new Set(),unsettledManifests=new WeakMap(),reconciling=new WeakSet();
  ...
  function pending(input,key,summary,reconcilable=false){const output=unsettled(input);if(reconcilable)unsettledManifests.set(output,Object.freeze({key,summary:Object.freeze(summary),input:Object.freeze({...input})}));return output;}
  async function reconcile(value={}){const manifest=unsettledManifests.get(value);if(!manifest||reconciling.has(value)||value.status!=='TRUSTED_TERMINAL_DELIVERY_UNSETTLED'||value.reasonCode!=='DURABLE_SETTLEMENT_REQUIRED'||value.terminalState!==manifest.input.terminalState||value.outcome!==manifest.input.outcome||value.sourceCount!==manifest.input.sourceCount||value.completionHash!==manifest.input.completionHash||value.transitionHash!==manifest.input.transitionHash)return blocked();reconciling.add(value);try{if(!await settle(manifest.key,manifest.summary))return value;unsettledManifests.delete(value);return reconciled(manifest.input);}finally{reconciling.delete(value);}}
  async function deliver(authorization={}){
    ...
    try{
      ...
      if(completion.status!=='TERMINAL_COMPLETION_CONFIRMED'){diagnostic('COMPLETION_DENIED');const summary={status:'TRUSTED_TERMINAL_DELIVERY_BLOCKED',outcome:null,stage:'COMPLETION_DENIED'};return await settle(key,summary)?blocked():pending({outcome},key,summary);}
      const transition=state.transition(completion,{...context,outcome});
      if(transition.status!=='TERMINAL_STATE_APPLIED'){diagnostic('TRANSITION_DENIED');const summary={status:'TRUSTED_TERMINAL_DELIVERY_BLOCKED',outcome:null,stage:'TRANSITION_DENIED',completionHash:completion.completionHash},input={outcome,sourceCount:completion.sourceCount,completionHash:completion.completionHash};return await settle(key,summary)?blocked():pending(input,key,summary);}
      ...
      const summary={status:output.status,terminalState:output.terminalState,outcome:output.outcome,transitionHash:output.transitionHash};if(!await settle(key,summary))return pending(output,key,summary,true);
      return output;
    }catch{diagnostic('TERMINAL_CHAIN_EXCEPTION');const summary={status:'TRUSTED_TERMINAL_DELIVERY_BLOCKED',outcome:null,stage:'TERMINAL_CHAIN_EXCEPTION'};return await settle(key,summary)?blocked():pending({outcome},key,summary);}
  }
