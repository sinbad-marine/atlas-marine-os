...
const authenticDeliveries=new WeakSet();
...
function deliveryPayload(value={}){return Object.freeze({version:String(value.version||''),status:String(value.status||''),reasonCode:value.reasonCode==null?null:String(value.reasonCode),transactionId:String(value.transactionId||''),contentType:String(value.contentType||''),renderingPolicy:String(value.renderingPolicy||''),answer:value.answer==null?null:String(value.answer),sources:Object.freeze((Array.isArray(value.sources)?value.sources:[]).map(source)),responseHash:value.responseHash==null?null:String(value.responseHash)});}
function adapt(input={}){
  ...
  const delivery=deliveryPayload({version:ADAPTER_VERSION,status:'DELIVERY_READY',reasonCode:null,transactionId:body.transactionId,contentType:CONTENT_TYPE,renderingPolicy:RENDERING_POLICY,answer:body.answer,sources:body.citations,responseHash:String(response.responseHash)});
  const output=Object.freeze({...delivery,deliveryHash:sha256(canonical(delivery))});authenticDeliveries.add(output);return output;
}
function isAuthenticDelivery(value){return Boolean(value&&typeof value==='object'&&authenticDeliveries.has(value));}
module.exports=Object.freeze({ADAPTER_VERSION,ORCHESTRATOR_VERSION,RESPONSE_SCHEMA,PROJECTOR_VERSION,RENDERING_POLICY,CONTENT_TYPE,canonical,sha256,responsePayload,deliveryPayload,adapt,isAuthenticDelivery});
