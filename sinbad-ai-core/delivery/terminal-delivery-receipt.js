...
function receiptPayload(value={}){return Object.freeze({version:String(value.version||''),status:String(value.status||''),reasonCode:value.reasonCode==null?null:String(value.reasonCode),transactionId:String(value.transactionId||''),sessionId:String(value.sessionId||''),channelId:String(value.channelId||''),outcome:String(value.outcome||''),attemptId:String(value.attemptId||''),authorizationHash:String(value.authorizationHash||''),deliveryHash:String(value.deliveryHash||''),sourceCount:Number.isInteger(value.sourceCount)&&value.sourceCount>=0&&value.sourceCount<=64?value.sourceCount:null});}
...
function record(authorization={},input={}){
  ...
  const body=receiptPayload({version:RECEIPT_VERSION,status:'DELIVERY_RECEIPT_RECORDED',reasonCode:null,transactionId:String(authorization.transactionId),sessionId,channelId,outcome,attemptId,authorizationHash:String(authorization.authorizationHash),deliveryHash:String(authorization.deliveryHash),sourceCount});
  if(!/^[a-f0-9]{64}$/u.test(body.authorizationHash)||!/^[a-f0-9]{64}$/u.test(body.deliveryHash))return blocked(authorization);
  recorded.add(authorization);const output=Object.freeze({...body,receiptHash:sha256(canonical(body))});authenticReceipts.add(output);return output;
}
function isAuthenticReceipt(value){return Boolean(value&&typeof value==='object'&&authenticReceipts.has(value));}
module.exports=Object.freeze({RECEIPT_VERSION,canonical,sha256,clean,receiptPayload,record,isAuthenticReceipt});
