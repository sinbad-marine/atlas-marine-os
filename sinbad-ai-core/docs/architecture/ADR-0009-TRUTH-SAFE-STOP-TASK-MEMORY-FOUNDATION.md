...
All `*Ref` fields are opaque identifiers, never filesystem paths, database keys, integrity proofs, or authority. A profile/template pair must match host-selected profile, template, owner and scope identities plus owner-approved source instruction, schema, validation rules, sensitivity, retention, and access-policy scope. Template creation cannot follow profile effectiveness; the profile cannot be future-effective or at/past its review boundary. A critical claim may be joined only to the host-selected task and stop identities, with `claim.observedAt <= execution.updatedAt <= stop.stoppedAt <= expectedNow`; the joint result explicitly carries no completion authority.
