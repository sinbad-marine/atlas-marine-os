...
All `*Ref` fields are opaque identifiers, never filesystem paths, database keys, integrity proofs, or authority. A profile/template pair must match owner-approved source instruction, schema, validation rules, sensitivity, retention, and access-policy scope. A critical claim may be joined only to a stopped execution and its four-field `SafeStopRecord`; the joint result explicitly carries no completion authority.
