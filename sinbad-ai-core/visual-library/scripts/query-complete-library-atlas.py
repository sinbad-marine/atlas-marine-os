...
def curated_stability_marks_query(value: str, limit: int) -> list[dict]:
    root=Path(__file__).resolve().parents[1]/"assets"/"curated-stability-marks-verified"; manifest=root/"manifest.json"
    if not manifest.is_file(): return []
    normalized=value.casefold(); groups={
        "plimsoll-load-line":("plimsoll","yükleme sınırı","yukleme siniri","fribord markası","fribord markasi","load line","freeboard mark"),
        "bow-draft-scale":("draft markaları","draft markalari","su çekimi skalası","su cekimi skalasi","baş draftı","bas drafti","draft scale","draught marks"),
        "inclining-experiment":("meyil deneyi","eğim deneyi","egim deneyi","ağırlık merkezi testi","agirlik merkezi testi","inclining experiment","inclining test"),
        "ballast-water-sampling":("balast suyu numunesi","balast suyu örnekleme","balast suyu ornekleme","balast denetimi","tuzluluk refraktometresi","ballast water sampling","ballast inspection")}
    preferred={k for k,phrases in groups.items() if any(p in normalized for p in phrases)}
    if not preferred:return []
    result=[]
    for item in json.loads(manifest.read_text(encoding="utf-8"))["visuals"]:
        if item["id"] not in preferred:continue
        path=root/item["file"]; digest=__import__("hashlib").sha256(path.read_bytes()).hexdigest()
        result.append({"visual_key":f"curated:stability-marks:{item['id']}","visual_type":"object","document_hash":"curated-stability-marks-verified","page_number":None,"image_number":None,"asset_hash":digest,"file":str(path),"title":item["credit"],"volume":None,"heading":item["heading"],"context":f"Verified ship stability and load-mark photograph. {item['license']}","topics":item["topics"],"sourcePaths":[item["sourceUrl"]],"rank":-2500.0,"assetUrl":f"http://127.0.0.1:31983/visuals/assets/{digest}.webp"})
    return result[:limit]
...
def query(db: sqlite3.Connection, value: str, limit: int, object_only: bool = False) -> list[dict]:
    ...
    if curated:
        return curated
    curated = curated_stability_marks_query(value, limit)
    if curated:
        return curated
...
def resolve_asset(db: sqlite3.Connection, atlas: Path, digest: str) -> dict:
    ...
    for path in hull_underwater_root.glob("*.webp"):
        if __import__("hashlib").sha256(path.read_bytes()).hexdigest()==digest:
            return {"asset_hash":digest,"file":str(path),"width":None,"height":None,"visual_type":"object","absolutePath":str(path.resolve())}
    stability_marks_root=Path(__file__).resolve().parents[1]/"assets"/"curated-stability-marks-verified"
    for path in stability_marks_root.glob("*.webp"):
        if __import__("hashlib").sha256(path.read_bytes()).hexdigest()==digest:
            return {"asset_hash":digest,"file":str(path),"width":None,"height":None,"visual_type":"object","absolutePath":str(path.resolve())}
    statements = [
