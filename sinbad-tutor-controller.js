...
(function(root){
  ...
  function submitTutorPrompt(prompt){const input=$('academyQuestion'),form=$('academyChatForm');if(!input||!form)return false;input.value=prompt;form.requestSubmit();return true}
  function renderCheck(){
    ...
    item.choices.forEach((choice,index)=>{const button=document.createElement('button');button.type='button';button.textContent=choice;button.addEventListener('click',()=>{[...host.querySelectorAll('button')].forEach(node=>node.disabled=true);state=root.SinbadTutorOrchestrator.advance(state.session,state.profile,{type:'ASSESSMENT',kind:'knowledge-check',score:index===item.answer?1:0,confidence:1});save();status(index===item.answer?`Doğru. ${item.explanation}`:`Yeniden çalışacağız. ${item.explanation}`);setTimeout(render,250)});host.append(button)});host.hidden=false;
  }
  function render(){
    if(!state)return;const action=state.action,advance=$('advanceTutorSession'),check=$('tutorKnowledgeCheck');advance.hidden=true;check.hidden=true;const heading=`${topicLabel(state.session.topicId)}\nHedef: ${objectiveLabel()}`;
    if(action.type==='EXPLAIN'){status(`${heading}\nSinbad açıklamayı hazırlıyor…`);advance.textContent='Bilgi kontrolüne geç';advance.hidden=false;if(!submitTutorPrompt(`${topicLabel(state.session.topicId)} konusunda şu öğrenme hedefini kaynaklara dayanarak, açık ve öğretici biçimde anlat: ${objectiveLabel()}`))status(`${heading}\nSohbet bağlantısı hazır değil. Oturum ilerletilmedi.`)}
    else if(action.type==='ASK_KNOWLEDGE_CHECK'){status(`${heading}\nKısa bilgi kontrolünü tamamlayın.`);renderCheck()}
    else if(action.type==='REMEDIATE'){status(`${heading}\nBu hedef henüz doğrulanmadı. Sinbad farklı bir açıklama hazırlıyor…`);advance.textContent='Yeniden kontrol et';advance.hidden=false;submitTutorPrompt(`${topicLabel(state.session.topicId)} konusundaki şu hedefi önceki açıklamadan farklı bir örnekle yeniden öğret: ${objectiveLabel()}`)}
  ...
  function advance(){if(!state||state.session.status!=='ACTIVE')return;const type=state.session.stage==='EXPLAIN'?'EXPLANATION_COMPLETE':'REMEDIATION_COMPLETE';state=root.SinbadTutorOrchestrator.advance(state.session,state.profile,{type});save();render()}
  function init(){
    ...
    $('startTutorSession').addEventListener('click',start);$('advanceTutorSession').addEventListener('click',advance);
  }
