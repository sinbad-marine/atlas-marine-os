...
(function(){
  ...
  let enabled=false,recognition=null,listening=false,awaitingAnswer=false,restartTimer=null,lastFinal='',listeningPurpose='question',turnGeneration=0;
  ...
  function stopRecognition(){clearRestart();if(!recognition)return;listening=false;lastFinal='';try{recognition.abort()}catch{}}
  ...
  function startRecognition(purpose){
    ...
    listeningPurpose=purpose;lastFinal='';
  ...
  function interruptSinbad(text){
    ...
    try{doc.getElementById('academyStopVoice')?.click()}catch{try{frame.contentWindow?.speechSynthesis?.cancel()}catch{}}
  ...
  function setupRecognition(){
    ...
    recognition.onend=()=>{const purpose=listeningPurpose,heard=lastFinal.trim();listening=false;if(purpose==='interrupt'){if(heard&&isInterruption(heard))interruptSinbad(heard);else if(enabled&&awaitingAnswer)setTimeout(startInterruptionListening,200);return}if(heard)submitTranscript(heard);else scheduleListening(300)};
  }
  function connectClassroom(){
    ...
    const resumeWhenReady=()=>{if(!enabled||!awaitingAnswer)return;const items=[...messages.querySelectorAll('.academy-message')],last=items.at(-1);if(items.length>previousCount&&last?.classList.contains('sinbad')){previousCount=items.length;const answerTurn=turnGeneration,deadline=Date.now()+8000;let observedSpeech=false;const waitForVoice=()=>{if(!enabled||answerTurn!==turnGeneration)return;const speaking=sinbadIsSpeaking()||stage.dataset.state==='speaking';observedSpeech=observedSpeech||speaking;if(speaking){startInterruptionListening();return setTimeout(waitForVoice,250)}if(stage.dataset.state==='thinking'&&Date.now()<deadline&&!observedSpeech)return setTimeout(waitForVoice,250);stopRecognition();awaitingAnswer=false;setStatus('Cevap tamamlandı. Yeni sorunuzu dinliyorum…');scheduleListening(250)};setTimeout(waitForVoice,250)}};
