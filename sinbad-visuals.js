(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.SinbadVisuals=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function rank(query,answer,documents){
    const wanted=tokens(`${query} ${answer}`);if(!wanted.length)return [];
    const ranked=[];
    for(const document of documents||[])for(const visual of document.visuals||[]){
      ...
      for(const occurrence of visual.occurrences||[]){
        ...
        const score=strong*4+weak;
        if(!best||score>best.score)best={occurrence,score,strong};
      }
    ...
    return ranked.sort((a,b)=>b.best.score-a.best.score||b.best.strong-a.best.strong||Number(b.visual.width)*Number(b.visual.height)-Number(a.visual.width)*Number(a.visual.height));
  }
  function project(item){const occurrence=item.best.occurrence,heading=occurrence.headings?.[0],title=item.collection||'American Practical Navigator',location=item.volume?`Cilt ${item.volume}, PDF sayfa ${occurrence.pdfPage}`:`PDF sayfa ${occurrence.pdfPage}`;return Object.freeze({src:`${item.assetBase||BASE}${item.visual.file}`,alt:heading||`${title}, ${location}`,caption:`${heading||title} — ${location}`,sourceUrl:item.visual.sourceUrl,sha256:item.visual.sha256});}
  ...
  async function bridgeSearch(query,answer,{fetcher=fetch,max=1}={}){const response=await fetcher(`${BRIDGE}/visuals/search`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query,answer,limit:max})});if(!response.ok)throw new Error(`Visual bridge returned ${response.status}`);const payload=await response.json();return (payload.visuals||[]).slice(0,max).map(bridgeProject);}
