(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.SinbadVisuals=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  async function bridgeSearch(query,answer,{fetcher=fetch,max=1}={}){const response=await fetcher(`${BRIDGE}/visuals/search`,{method:'POST',headers:{'Content-Type':'application/json',...argosBridgeHeaders('VISUAL_SEARCH','/visuals/search')},body:JSON.stringify({query,answer,limit:Math.max(max,3)})});if(!response.ok)throw new Error(`Visual bridge returned ${response.status}`);const payload=await response.json();return (payload.visuals||[]).filter(item=>['object','chart-table','chart-table-highlight'].includes(item.visual_type)).slice(0,max).map(bridgeProject);}
  async function select(query,answer,{fetcher,max=1,documents}={}){if(documents){try{return rank(query,answer,documents).slice(0,max).map(project);}catch(error){console.warn('Sinbad visual atlas unavailable',error);return [];}}const selected=[];try{selected.push(...await bridgeSearch(query,answer,{fetcher,max}));}catch(error){console.warn('Sinbad complete visual atlas unavailable',error);}if(selected.length<max)try{selected.push(...rank(query,answer,await load(fetcher)).map(project));}catch(error){console.warn('Sinbad visual atlas unavailable',error);}return [...new Map(selected.map(item=>[item.sha256||item.src,item])).values()].slice(0,max);}
